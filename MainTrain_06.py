...
class DNN_BS_hyb_OFDM(nn.Module):  # 输入所有用户的KB比特反馈信息,输出预编码
    ...
    def forward(self, h):
        ...
        F_imag = (torch.matmul(RF_imag, BB_real) + torch.matmul(RF_real, BB_imag)).reshape(Nc, -1, K * Nt) # bug modified
...
def train(Nc, N, Nt, B, Nr, L, SNR_dB, K, EPOCH, BATCH_SIZE):  # N代表路径数，若N为-1则代表
    ...
    for epoch in range(EPOCH):
        ...
        if test_SE > best_SE:
            best_SE = test_SE
            torch.save(net_BS,
                       './Data_driven_' + str(B) + 'B' + str(N) + 'Ncl' + str(L) + 'L' + str(K) + 'K' + '_8L_UPA_T01_Trans_warm.pth')
            print('Model saved!')
