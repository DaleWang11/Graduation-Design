...
class DNN_US_RF_OFDM(nn.Module):  # 输入信道输出 经过导频相乘再加上噪声后 在用神经网络压缩量化B比特反馈信息
...
def train(Nc, N, Nt, B, Nr, L, SNR_dB, K, EPOCH, BATCH_SIZE):  # N代表路径数，若N为-1则代表
    ...
    for epoch in range(EPOCH):
        ...
        if test_SE > best_SE:
            best_SE = test_SE
            torch.save(net_BS,
                       './net_BS_dif_B_' + str(B) + 'B' + str(N) + 'Ncl' + str(L) + 'L' + str(K) + 'K' + '_8L_UPA_T01_Trans_warm.pth')
            print('Model saved!')
